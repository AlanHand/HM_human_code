                        ...
                        if Mode == 0:
                            ...
                            if lastMode != Mode:
                                ...
                                if lastPID is not None and get_data.read_data()[0] != 'K':
                                    os.kill(lastPID, signal.SIGKILL)
                                ChildCvFindStream = subprocess.Popen(["python3", "/home/pi/human_code/HM_cv_find_stream.py"])  # 自动射门
                            ...
                            if lastMode != Mode:
                                ...
                                if lastPID is not None and get_data.read_data()[0] != 'K':
                                    os.kill(lastPID, signal.SIGKILL)
                                ChildColorStream = subprocess.Popen(["python3", "/home/pi/human_code/HM_cv_color_stream.py"])  # 颜色识别
                            ...
                            if lastMode != Mode:
                                ...
                                if lastPID is not None and get_data.read_data()[0] != 'K':
                                    os.kill(lastPID, signal.SIGKILL)
                                ChildCvFindHand = subprocess.Popen(["python3", "/home/pi/human_code/HM_cv_find_hand.py"])  # 手势识别
                            ...
                            if lastMode != Mode:
                                ...
                                if lastPID is not None and get_data.read_data()[0] != 'K':
                                    os.kill(lastPID, signal.SIGKILL)
                                ChildCvLinePatrol = subprocess.Popen(["python3", "/home/pi/human_code/HM_cv_line_patrol.py"]) # 寻线
                            ...
                            if lastMode != Mode:
                                ...
                                if lastPID is not None and get_data.read_data()[0] != 'K':
                                    os.kill(lastPID, signal.SIGKILL)
                                ChildCvTrackStream = subprocess.Popen(["python3", "/home/pi/human_code/HM_cv_track_stream.py"])  # 云台跟踪
